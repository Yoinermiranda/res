      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-5 lg:px-8">
        ...
        <header className={`flex flex-col gap-4 rounded-2xl border p-4 lg:flex-row lg:items-center lg:justify-between ${surface} ${border}`}>
          ...
          <nav className="flex flex-wrap items-center gap-1.5">
            ...
            <button onClick={() => setDarkMode(!darkMode)} title={darkMode ? 'Modo claro' : 'Modo oscuro'} className={['rounded-xl px-3 py-2 text-sm font-bold transition-all duration-200', darkMode ? 'bg-amber-400/[.12] text-amber-400 hover:bg-amber-400/[.22]' : 'bg-slate-800/[.07] text-slate-600 hover:bg-slate-800/[.13]'].join(' ')}>{darkMode ? '☀️' : '🌙'}</button>

            <button onClick={handleLogout} className={['rounded-xl px-3.5 py-2 text-sm font-bold transition-all duration-200', darkMode ? 'bg-white/[.05] text-slate-400 hover:bg-red-500/[.12] hover:text-red-400' : 'bg-slate-900 text-white hover:bg-slate-700'].join(' ')}>Salir</button>
          </nav>
        </header>
        ...
        {loading ? (
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {orders.map((order) => {
              ...
              return (
                <div key={order.id} className={['flex flex-col rounded-2xl border transition-all duration-300', surface, allDone ? urgency.ring + ' shadow-lg' : urgency.ring + ' shadow-sm', isNew ? 'ring-2 ring-orange-500 ring-offset-2 scale-[1.02] shadow-xl shadow-orange-500/25' : ''].join(' ')}>
                  {isNew && (
                  ...
                  <div className="flex-1 space-y-2 p-4">
                    {order.items.map((item) => (
                      <button key={item.id} type="button" onClick={() => handleItemDone(order.id, item.id)} className={['group w-full rounded-xl border px-4 py-3 text-left', 'transition-all duration-200 hover:-translate-y-px active:scale-[.98]', item._done ? darkMode ? 'border-emerald-500/30 bg-emerald-500/[.08]' : 'border-emerald-200 bg-emerald-50' : darkMode ? `border-white/[.07] bg-white/[.03] hover:border-orange-500/20 ${surface2}` : 'border-slate-100 bg-slate-50 hover:border-orange-200 hover:bg-orange-50/40'].join(' ')}>
                        <div className="flex items-center justify-between gap-3">
                          <div className="flex items-center gap-3 min-w-0">
                            <span className={['flex h-6 w-6 shrink-0 items-center justify-center rounded-lg text-[10px] font-black', item._done ? 'bg-emerald-500 text-white' : darkMode ? 'bg-white/[.10] text-slate-400' : 'bg-slate-200 text-slate-500'].join(' ')}>
                              {item._done ? '✓' : item.cantidad}
                            </span>
                            <span className={['truncate text-sm font-bold', item._done ? darkMode ? 'text-emerald-400 line-through' : 'text-emerald-600 line-through' : heading].join(' ')}>
                              {item.cantidad > 1 && !item._done ? `${item.cantidad}× ` : ''}{item.producto?.nombre}
                            </span>
                          </div>
                          {item._done && (
                            <span className={`shrink-0 text-[9px] font-black uppercase tracking-widest ${darkMode ? 'text-emerald-500' : 'text-emerald-600'}`}>Listo</span>
                          )}
                        </div>
                        {item.notas_preparacion && (
                          <p className={`mt-1.5 pl-9 text-[11px] ${item._done ? (darkMode ? 'text-emerald-500/60' : 'text-emerald-500/70') : muted}`}>{item.notas_preparacion}</p>
                        )}
                      </button>
                    ))}
                  </div>
                  ...
                  <div className={`border-t px-4 pb-4 pt-3 ${borderB}`}>
                    ...
                    <button type="button" disabled={!allDone} onClick={() => handleOrderComplete(order.id)} className={['w-full rounded-xl py-3 text-sm font-bold transition-all duration-200', allDone ? 'bg-orange-500 text-white shadow-md shadow-orange-500/20 hover:bg-orange-400 hover:-translate-y-px active:scale-[.98]' : 'cursor-not-allowed opacity-40 ' + (darkMode ? 'bg-white/[.06] text-slate-400' : 'bg-slate-100 text-slate-400')].join(' ')}>
